...
FEditorPackImporter::FEditorPackImporter()
{
}
...
bool FEditorPackImporter::ImportPack()
{
	...
	DesktopPlatform->OpenFileDialog(
		ParentWindowPtr,
		TEXT("Select a pack file to import..."),
	...
	{
		LINGO_LOG(EVerbosityLevel::Debug, TEXT("Pack import cancelled by user"));
		return false;
	}

	// Extract pack file
	if (!ExtractPackToTemp(OutFilePaths[0], TempDir))
	{
		LINGO_LOG(EVerbosityLevel::Error, TEXT("Failed to extract pack file: %s"), *OutFilePaths[0]);
		return false;
	}
	...
	LINGO_LOG(EVerbosityLevel::Debug, TEXT("Pack %s was successfully imported! (%d ONNX files, %d other files)"),
	       *FPaths::GetCleanFilename(OutFilePaths[0]), OnnxFilesImported, OtherFilesMoved);

	return true;
}

bool FEditorPackImporter::ExtractPackToTemp(const FString& PackFilePath, const FString& TempDir)
{
	using Thespeon::Core::IO::RuntimeFileLoader;
	...
	FString Command = FString::Printf(
		...
		*FPaths::ConvertRelativePathToFull(PackFilePath),
		*TempDir
	);
...
int32 FEditorPackImporter::ImportOnnxFiles(const FString& TempDir)
{
	using Thespeon::Core::IO::RuntimeFileLoader;
	...
	if (OnnxFiles.Num() == 0)
	{
		LINGO_LOG(EVerbosityLevel::Debug, TEXT("No .onnx files found under: %s"), *TempDir);
		return 0;
	}
	...
	for (const FString& AbsPath : OnnxFiles)
	{
		...
		Task->bSave = true;     // auto-save the package
...
int32 FEditorPackImporter::MoveOtherFiles(const FString& TempDir)
{
	using Thespeon::Core::IO::RuntimeFileLoader;
	...
	FM.FindFilesRecursive(OtherFiles, *TempDir, TEXT("*.json"), /*Files=*/true, /*Directories=*/false);
