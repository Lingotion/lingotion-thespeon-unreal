...
FLingotionLanguage Thespeon::Core::BestLanguageMatch(const TArray<FLingotionLanguage>& Candidates, const FLingotionLanguage& Query)
{
    ...
    {
        if (Candidate.IsUndefined()) { continue; } // defensive
    
        // Primary score: language
        const int LangScore =
            Candidate.ISO639_2.Equals(Query.ISO639_2, ESearchCase::IgnoreCase) ? 10 : 0; //to be expanded
    
        // Secondary score: region (tie-breaker)
        const int RegionScore =
            Candidate.ISO3166_1.Equals(Query.ISO3166_1, ESearchCase::IgnoreCase) ? 10 : 0; //to be expanded
...
bool FLingotionLanguage::TryParseFromJson(
    const TSharedPtr<FJsonObject>& Json,
    FLingotionLanguage& OutLanguage)
{
    if (!Json.IsValid())
        return false;
