...
namespace Thespeon
{
    ...
    {
        /// @brief Represents a runtime lookup table that can dynamically add entries and check for existing keys.
        /// Manages both static (pre-loaded) and dynamic (runtime-added) grapheme-to-phoneme mappings.
        class RuntimeLookupTable
        {
        public:
            /// @brief Initializes a new instance of the RuntimeLookupTable with a static lookup table.
            ...
            /// @brief Tries to get the value associated with the specified key from the lookup tables.
            ...
            /// @brief Checks if the specified key exists in either the static or dynamic lookup tables.
            /// @param Key The key to check for existence
            /// @return True if the key exists in either table, otherwise false
            bool ContainsKey(const FString& Key) const;
            
            /// @brief Adds or updates an entry in the dynamic lookup table.
