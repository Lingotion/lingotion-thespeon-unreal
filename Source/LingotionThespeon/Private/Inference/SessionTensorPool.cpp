...
{
    if (TUniquePtr<ModelIOData>* Found = TensorMap.Find(Name)) { Out = Found->Get(); return true; }
...
{
    if (const TUniquePtr<ModelIOData>* Found = TensorMap.Find(Name)) { Out = Found->Get(); return true; }
    Out = nullptr; return false;
}
